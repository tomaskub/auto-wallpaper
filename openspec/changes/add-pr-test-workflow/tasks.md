...
- [ ] 1.1 Create `.github/workflows/pull-request-tests.yml` with a stable workflow and job name, a `pull_request` trigger limited to `main`, and `contents: read` permissions.
- [ ] 1.2 Configure one macOS 15 job that checks out the pull request head and runs `swift test`.
- [ ] 1.3 Add pull-request-scoped concurrency with cancellation so a new commit supersedes an older in-progress run.

## 2. Validate the workflow

- [ ] 2.1 Validate the workflow YAML and run `swift test` locally to confirm the planned CI command passes the complete package test suite.
